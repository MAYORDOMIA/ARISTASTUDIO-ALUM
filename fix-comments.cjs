...
function fixFile(file) {
  ...
  content = content.replace(/(\/\/[^\n]*?)\s+(return |const |let |if |try |await |document\.|set|setActive|window\.|console\.|_set|update|this\.|for |while |export )/g, '$1\n$2');
  
  // Specific fixes if the generic one misses
  content = content.replace(/\/\/ Opción de Super Admin oculta para todos excepto para ti if/g, '// Opción de Super Admin oculta para todos excepto para ti\n if');
  // Enforce uniqueness return
  content = content.replace(/\/\/ Enforce uniqueness return/g, '// Enforce uniqueness\n return');
  // Suscripción en tiempo real para sincronización de activación useEffect
  content = content.replace(/\/\/ Suscripción en tiempo real para sincronización de activación useEffect/g, '// Suscripción en tiempo real para sincronización de activación\n useEffect');
