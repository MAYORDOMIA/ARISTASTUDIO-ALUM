...
const App: React.FC = () => {
  ...
  const setRecipes = (
    ...
    _setRecipes((prev) => {
      const next = typeof updater === "function" ? updater(prev) : updater; // Enforce uniqueness
      return Array.from(
        new Map<string, ProductRecipe>(next.map((r) => [r.id, r])).values(),
      );
    });
  };
  ...
  useEffect(() => {
    ...
    /* Suscribirse a cambios en el perfil (para activación) y en presupuestos */ let channel: any;
    try {
      channel = supabase
          ...
          (payload: any) => {
            ...
            if (payload.new) {
              ...
              if (payload.new.is_active) {
                /* Re-evaluar acceso a nivel local si el admin activó la cuenta */ checkDeviceAccess(
                  payload.new,
                );
              }
            }
    ...
    try {
      const [
        ...
        supabase
          ...
          .order("detail", { ascending: true })
          .limit(50000),
        supabase.from("tratamientos_usuario").select("*").eq("user_id", userId).order("name", { ascending: true }).limit(50000),
        supabase.from("paneles_usuario").select("*").eq("user_id", userId).order("detail", { ascending: true }).limit(50000),
        supabase.from("dvh_usuario").select("*").eq("user_id", userId).order("detail", { ascending: true }).limit(50000),
        supabase.from("recetas_usuario").select("*").eq("user_id", userId).order("name", { ascending: true }).limit(50000),
      ...
      return {
        aluminum: cleanData(aluRes.data).map((x) => ({
          ...
          treatmentCost: x.treatment_cost,
          thickness: x.thickness,
          isGlazingBead: x.is_glazing_bead,
          glazingBeadStyle: x.glazing_bead_style,
          minGlassThickness: x.min_glass_thickness,
          maxGlassThickness: x.max_glass_thickness,
          id: x.master_ref || x.id,
        })),
        glasses: cleanData(glsRes.data).map((x) => ({
          ...x,
          pricePerM2: x.price_per_m2,
          thickness: x.thickness,
          isMirror: x.is_mirror,
          id: x.master_ref || x.id,
        })),
        accessories: cleanData(accRes.data).map((x) => ({
          ...x,
          unitPrice: x.unit_price,
          id: x.master_ref || x.id,
        })),
        treatments: cleanData(trtRes.data).map((x) => ({
          ...x,
          pricePerKg: x.price_per_kg,
          hexColor: x.hex_color,
          id: x.master_ref || x.id,
        })),
  ...
  const fetchProfile = async (user: any) => {
    ...
    try {
      ...
      /* Si no existe, el trigger lo debería haber creado, pero por seguridad verificamos */ if (
        checkError ||
        !profileCheck
      ) {
        console.warn(
          "Perfil no encontrado, esperando al trigger o reintentando...",
        );
        /* Pequeño retardo para dar tiempo al trigger de auth.users */ await new Promise(
          (r) => setTimeout(r, 1000),
        );
        ...
        if (!profileCheck) {
          console.log(
            "Perfil no encontrado tras reintento. Creando desde frontend...",
          );
  ...
  const handleEditQuoteItem = (item: QuoteItem) => {
    setTriggerQuoterAction({ action: "editar", ts: Date.now(), item });
    setActiveTab("quoter");
  };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        localStorage.setItem(storageKey, stringified); // Guardar configuración en la nube
        if (isSupabaseConfigured && session?.user?.id) {
          await supabase
            .from("configuracion_usuario")
            .upsert([{ user_id: session.user.id, config_data: config }], {
              onConflict: "user_id",
            });
        }
      } catch (e: any) {
      ...
      <div className="flex h-screen items-center justify-center bg-[#f1f5f9] p-4">
        <div className="text-center p-8 bg-white rounded-3xl shadow-xl border border-slate-200 max-w-md w-full">
          ...
          <p className="text-slate-500 text-sm mb-8">
            Tu cuenta ha sido creada exitosamente, pero un administrador debe
            activarla para que puedas acceder al cotizador.
          </p>
          <button
      ...
      <div className="flex h-screen items-center justify-center bg-[#f1f5f9] p-4">
        <div className="text-center p-8 bg-white rounded-3xl shadow-xl border border-slate-200 max-w-md w-full">
          ...
          <p className="text-slate-500 text-sm mb-4">
            No se pudo cargar tu perfil correctamente. El proceso de registro
            pudo haber sido interrumpido.
          </p>
          <div className="bg-red-50 text-red-600 p-3 rounded-xl text-xs font-mono mb-8 text-left overflow-hidden text-ellipsis">
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f1f5f9] text-[#0f172a] transition-colors duration-300">
      ...
      >
        <div className="p-4 flex items-center justify-between border-b border-slate-100 overflow-hidden">
          {(isSidebarOpen || window.innerWidth >= 1024) && (
            <div
              className={`flex items-center gap-2 transition-opacity duration-300 ${!isSidebarOpen && "lg:opacity-0"}`}
            >
              <div className="w-8 h-8 bg-sky-500 rounded-xl flex items-center justify-center font-black text-white shadow-lg text-lg italic">
                A
              </div>
            </div>
          )}
          <button
            ...
            className="p-2 hover:bg-slate-50 rounded-xl text-slate-400 transition-all hover:text-sky-500"
          >
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto custom-scrollbar">
          {MENU_ITEMS.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-4 p-3 rounded-xl transition-all group border ${activeTab === item.id ? "bg-sky-500 text-white font-black shadow-lg border-sky-600" : "text-gray-500 hover:bg-slate-50 hover:text-sky-500 border-transparent hover:border-slate-100 "}`}
            >
              ...
              {isSidebarOpen && (
                <span className="text-[11px] truncate uppercase font-black tracking-wider">
                  {item.label}
                </span>
              )}
            </button>
          ...
          {isSuperAdmin && (
            <button
              ...
              className={`w-full flex items-center gap-4 p-3 rounded-xl transition-all group border ${activeTab === "admin" ? "bg-red-500 text-white font-black shadow-lg border-red-600" : "text-red-500 hover:bg-red-50 border-transparent"}`}
            >
              <ShieldCheck size={18} />
              {isSidebarOpen && (
        ...
        <div className="p-4 border-t border-slate-100 ">
          <div
            className={`flex items-center gap-2 p-2 rounded-xl border transition-all text-[9px] font-black uppercase tracking-wider ${isSaving ? "text-sky-500 border-sky-100 bg-sky-50/50 " : "text-green-600 border-green-100 bg-green-50/50 "}`}
          >
            {isSaving ? (
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative w-full">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 lg:px-6 shadow-sm z-10 transition-colors">
          ...
          <div className="flex items-center gap-3 lg:gap-6">
              ...
              <div className="hidden sm:flex items-center gap-2 animate-in fade-in slide-in-from-right-2">
                {openingName && (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-sky-50 rounded-lg border border-sky-100 ">
                    ...
                    <span className="text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-sky-600 ">
                      {openingName}
                    </span>
                  </div>
                )}
                <button
                  onClick={() =>
                    setTriggerQuoterAction({ action: "nuevo", ts: Date.now() })
                  }
                  className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-[9px] lg:text-[10px] font-black uppercase tracking-widest transition-colors flex items-center gap-1.5"
                >
            ...
            <div className="flex items-center gap-3 lg:gap-4">
              <div className="flex flex-col items-end">
                <span className="text-[7px] lg:text-[8px] text-slate-400 uppercase font-black tracking-widest">
                  P. ALU
                </span>
                <span className="text-xs lg:text-sm font-mono text-sky-600 font-black">
                  ${(config.aluminumPricePerKg || 0).toFixed(2)}
                </span>
              </div>
              <div className="hidden sm:flex flex-col items-end pl-3 lg:pl-4 border-l border-slate-100 ">
                <span className="text-[7px] lg:text-[8px] text-slate-400 uppercase font-black tracking-widest">
                  MARGEN
                </span>
                <span className="text-xs lg:text-sm font-mono text-emerald-600 font-black">
                  +{config.laborPercentage}%
                </span>
              </div>
              <div className="pl-3 lg:pl-4 border-l border-slate-100 ">
                <button
                  onClick={() => {
        ...
        <section className="flex-1 overflow-y-auto p-4 lg:p-6 custom-scrollbar bg-[#f8fafc] ] transition-colors">
          <div className={activeTab === "database" ? "h-full" : "hidden"}>
            <DatabaseCRUD
              aluminum={aluminum}
              setAluminum={setAluminum}
              glasses={glasses}
            ...
            <div className="max-w-4xl mx-auto space-y-2 animate-in fade-in slide-in-from-bottom-2">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="bg-white ] p-3 rounded-2xl border border-slate-200 shadow-sm space-y-2">
                  <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-sky-600 flex items-center gap-2 border-b pb-1">
                    <Zap size={14} /> Económicos y Técnicos
                  </h2>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-0.5">
                      ...
                      <input
                        ...
                        value={config.aluminumPricePerKg ?? ""}
                        onChange={(e) =>
                          setConfig({
                            ...config,
                    ...
                    <div className="space-y-0.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setConfig({
                            ...config,
                            laborPercentage: parseFloat(e.target.value) || 0,
                          })
                        }
                      />
                    </div>
                    ...
                    <div className="space-y-0.5">
                      ...
                      <input
                        type="number"
                        className="w-full bg-slate-50 border p-1 rounded-lg font-mono text-xs font-bold "
                        value={config.discWidth ?? ""}
                        onChange={(e) =>
                          setConfig({
                            ...config,
                            discWidth: parseFloat(e.target.value) || 0,
                          })
                        }
                      />
                    </div>
                    <div className="space-y-0.5">
                      ...
                      <input
                        type="number"
                        className="w-full bg-slate-50 border p-1 rounded-lg font-mono text-xs font-bold "
                        value={config.blindPanelPricePerM2 ?? ""}
                        onChange={(e) =>
                          setConfig({
                            ...config,
                    ...
                    <div className="space-y-0.5">
                      <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-1 flex items-center gap-1">
                        <Wind size={8} /> Tela Mosq. ($/M2)
                      </label>
                      <input
                        type="number"
                        className="w-full bg-slate-50 border p-1 rounded-lg font-mono text-xs font-bold "
                        value={config.meshPricePerM2 ?? ""}
                        onChange={(e) =>
                          setConfig({
                            ...config,
                            meshPricePerM2: parseFloat(e.target.value) || 0,
                          })
                        }
                      />
                    </div>
                    <div className="space-y-0.5">
                      ...
                      <input
                        type="number"
                        className="w-full bg-slate-50 border p-1 rounded-lg font-mono text-xs font-bold "
                        value={config.handrailExtraIncrement ?? ""}
                        onChange={(e) =>
                          setConfig({
                            ...config,
                            handrailExtraIncrement:
                              parseFloat(e.target.value) || 0,
                          })
                        }
                      />
                    ...
                    <div className="space-y-0.5">
                      ...
                      <input
                        type="number"
                        className="w-full bg-slate-50 border p-1 rounded-lg font-mono text-xs font-bold "
                        value={config.mamparaExtraIncrement ?? ""}
                        onChange={(e) =>
                          setConfig({
                            ...config,
                            mamparaExtraIncrement:
                              parseFloat(e.target.value) || 0,
                          })
                        }
                      />
                ...
                <div className="bg-white ] p-3 rounded-2xl border border-slate-200 shadow-sm space-y-2">
                  <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-sky-600 flex items-center gap-2 border-b pb-1">
                    <Building2 size={14} /> Identidad Corporativa
                  </h2>
                  <div className="space-y-1">
                    <div className="space-y-0.5">
                      ...
                      <input
                        type="text"
                        className="w-full bg-slate-50 border p-1 rounded-lg text-xs font-bold "
      ...
      <style>{` .report-btn { display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 8px 12px; border-radius: 12px; transition: all 0.2s; border: 1px solid transparent; } .report-btn:hover { background-color: #f1f5f9; border-color: #e2e8f0; } .dark .report-btn:hover { background-color: #1e293b; border-color: #334155; } .icon-style { color: #64748b; transition: color 0.2s; } .report-btn:hover .icon-style { color: #0ea5e9; } .label-style { font-size: 7px; font-weight: 900; color: #94a3b8; letter-spacing: 0.1em; transition: color 0.2s; } .report-btn:hover .label-style { color: #0ea5e9; } `}</style>
    </div>
  );
};
export default App;
